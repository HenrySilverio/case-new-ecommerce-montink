...
import { XMarkIcon, PlusIcon, MinusIcon, TrashIcon, ClockIcon, MagnifyingGlassIcon, ShoppingCartIcon } from '@heroicons/react/24/outline';
...
const CartDrawer: React.FC = () => {
  ...
  const handleCepLookup = useCallback(async (cepToLookup: string) => {
    ...
    try {
      const fetchedAddress: ViaCepResponse | null = await fetchAddressByCep(cepToLookup);
      if (fetchedAddress && !fetchedAddress.erro) {
        ...
        setCepLookupError(fetchedAddress?.erro ? "CEP não encontrado." : "Erro ao buscar CEP. Tente novamente.");
      }
    } catch (error) {
      setAddress(null);
      setPersistedCep(null);
  ...
  const cartContent = (
    <div className="h-full max-h-[62.5rem] flex flex-col text-gray-800 dark:text-brand-off-white overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
        <label htmlFor="cepInputCart" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Consultar Frete e Entrega:</label>
        <div className="flex items-center">
          ...
          >
            {isLoadingCep ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div> : <MagnifyingGlassIcon className="h-5 w-5"/>}
          </button>
        </div>
        {cepLookupError && <p className="text-xs text-red-600 dark:text-red-400 mt-1">{cepLookupError}</p>}
        {address && (
        ...
        {items.length === 0 ? (
          <div className="flex-grow flex items-center justify-center p-4 h-full">
            <div className="text-center">
              ...
              {!persistedCep && <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Adicione seu CEP acima para ver estimativas de entrega.</p>}
            </div>
          </div>
        ) : (
          <div className="p-4 space-y-3">
            {items.map((item) => (
              <div key={item.id} className="flex items-start space-x-3 p-2 bg-white dark:bg-zinc-800 rounded shadow">
                ...
                <button onClick={() => handleRemoveItem(item.id)} className="p-1 text-gray-500 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-500 flex-shrink-0">
